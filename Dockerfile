...
# FROM julio471/jupter-hadoop-pets:7.0
...
RUN apt-get clean && \
    ...
    echo "${NB_USER} ALL=(ALL) NOPASSWD: /usr/sbin/service postgresql *, /usr/bin/pg_ctlcluster *, /usr/sbin/sshd, /usr/bin/jps" > /etc/sudoers.d/notebook && \
